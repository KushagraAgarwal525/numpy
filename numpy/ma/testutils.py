...
def assert_equal(actual, desired, err_msg=''):
    ...
    if not (isinstance(actual, ndarray) or isinstance(desired, ndarray)):
        # Keep ``==`` for historical dtype/scalar comparisons used by ma tests
        # (e.g. dtype('S8') == '|S8'), but treat NaN/NaT as equal like
        # numpy.testing.assert_equal (gh-6661).
        if actual == desired:
            return
        try:
            if np.isnan(actual) and np.isnan(desired):
                return
        except TypeError:
            pass
